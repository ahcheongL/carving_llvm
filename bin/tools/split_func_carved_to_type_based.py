...
shutil.rmtree(carved_dir + "_type_based", ignore_errors=True)
...
for fn in glob.glob("{}/*/*".format(carved_dir)):
    ...
    if "_type_based" in fn:
        continue

    filename = fn.split("/")[-1]
    if filename[:12] == "carved_file_":
        try:
            os.mkdir(carved_dir + "_type_based/" + filename)
        except:
            pass
        existing_files = glob.glob(carved_dir + "_type_based/" + filename + "/*")
        num_existing_files = len(existing_files)
        for carved_file_fn in glob.glob(fn + "/*"):
            shutil.copy(
                carved_file_fn,
                carved_dir + "_type_based/" + filename + "/" + str(num_existing_files),
            )
    ...
    if func_name not in type_info:
        print("{} not in type_info".format(func_name))
        continue
    ...
    with open(fn, "r") as f1:
        ...
        if len(variables) != len(var_types):
            print(
                "{} has {} variables, but {} types".format(
                    func_name, len(variables), len(var_types)
                )
            )
            continue
        ...
        for varinfo in variables:
            ...
            with open(
                ...
                for ptr_line in result_ptrs:
                    ...
                    f2.write(
                        "{}:{}".format(new_ptr_index, ":".join(ptr_line.split(":")[1:]))
                    )
...
for type_name in types:
    ...
    if len(carved_obj_fns) == 0:
        print("Removing {}".format(type_dirn))
        os.rmdir(type_dirn)
