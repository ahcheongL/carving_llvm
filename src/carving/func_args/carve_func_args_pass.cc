...
bool CarverFAPass::runOnModule(llvm::Module &M) {
  llvm::outs() << "Running CarverFAPass\n";
...
const char *
#endif
CarverFAPass::getPassName() const {
  return "carving function argument instrumentation";
}

void CarverFAPass::insert_global_carve_probe(llvm::Function *F) {
  ...
  if (search != global_var_uses.end()) {
    for (auto glob_iter : search->second) {
      ...
      insert_carve_probe_fa(glob_val);
    }
  }
...
// target_funcs.txt.
void CarverFAPass::get_instrument_func_set() {
...
void CarverFAPass::instrument_func(llvm::Function *func) {
  ...
  if (demangled_func_name == "main") {
    ...
    for (auto &arg_iter : func->args()) {
      ...
      insert_carve_probe_fa(func_arg);
    }
    ...
    for (auto iter : input_files) {
      llvm::Constant *input_name_const = gen_new_string_constant(iter, IRB);
...
void CarverFAPass::insert_carve_probe_fa(llvm::Value *val) {
  ...
  if (val_type == Int1Ty) {
    ...
    for (auto _iter : memberoffsets) {
      ...
      insert_carve_probe_fa(extracted_val);
      idx++;
    }
    ...
    if (is_class_type) {
      ...
      insert_gep_carve_probe_fa(elem_ptr);
...
void CarverFAPass::instrument_main(llvm::Function *main_func) {
...
void CarverFAPass::insert_gep_carve_probe_fa(llvm::Value *gep_val) {
  ...
  if (gep_pointee_type->isStructTy()) {
    insert_struct_carve_probe_fa(gep_val, gep_pointee_type);
  } else if (gep_pointee_type->isArrayTy()) {
    insert_array_carve_probe_fa(gep_val);
    ...
    insert_carve_probe_fa(loadval);
  }

  return;
}

void CarverFAPass::insert_array_carve_probe_fa(llvm::Value *arr_ptr_val) {
  ...
  insert_gep_carve_probe_fa(getelem_instr);
  loopblock = IRB->GetInsertBlock();
...
void CarverFAPass::insert_struct_carve_probe_fa(llvm::Value *struct_ptr,
                                                llvm::Type *type) {
  ...
  if (search2 == class_name_map.end()) {
    insert_struct_carve_probe_fa_inner(struct_ptr, type);
  } else {
...
void CarverFAPass::insert_struct_carve_probe_fa_inner(llvm::Value *struct_ptr,
                                                      llvm::Type *type) {
...
void CarverFAPass::gen_class_carver_fa() {
  class_carver =
  ...
  for (auto class_type : class_name_map) {
    ...
    insert_struct_carve_probe_fa_inner(casted_var, class_type_ptr);
    IRB->CreateRetVoid();
  }
...
void CarverFAPass::insert_alloca_probe(llvm::BasicBlock &entry_block) {
...
void CarverFAPass::insert_check_carve_ready() {
...
void CarverFAPass::insert_dealloc_probes() {
...
bool CarverFAPass::insert_mem_func_call_probe(llvm::Instruction *call_inst,
...
Constant *CarverFAPass::get_mem_alloc_type(llvm::Instruction *call_inst) {
  if (call_inst == NULL) {
...
static llvm::RegisterPass<CarverFAPass> X("carve", "Carve pass", false, false);

static void registerPass(const llvm::PassManagerBuilder &,
                         llvm::legacy::PassManagerBase &PM) {
  auto p = new CarverFAPass();
  PM.add(p);
}
